...
echo "Starting Growth Stage Inference Job: ${JOB_NAME}, ID: ${JOB_ID}"
start_time=$(date +%s)
...
case "$PLATFORM" in
  gcp|aws|azure)
    ...
    JOB_MEMORY="${MEMORY}"
    LOCAL_INPUT_DIR="/data/input/plot_patches"
    ...
    EFFECTIVE_OUTPUT_PATH="$LOCAL_OUTPUT_DIR/$(basename $OUTPUT_PATH_GS)"

    echo "Downloading plot patches from ${INPUT_DIR}"
  ...
  slurm)
    ...
    EFFECTIVE_OUTPUT_PATH="$OUTPUT_PATH_GS"
    EFFECTIVE_MODEL_PATH="$MODEL_PATH"
...
# --- 3. CORE LOGIC: Run the main Python script for growth stage inference ---
echo "Starting growth stage inference"
echo "Input: ${EFFECTIVE_INPUT_DIR}, Output: ${EFFECTIVE_OUTPUT_PATH}, Model: ${EFFECTIVE_MODEL_PATH}"
...
# Change to execution directory so config files can be found
cd /app/execution
...
echo "Growth stage inference finished."
...
if [ -f "$EFFECTIVE_OUTPUT_PATH" ]; then
...
case "$PLATFORM" in
  gcp|aws|azure)
    echo "Uploading results to ${OUTPUT_PATH_GS}"
    rclone copy "$EFFECTIVE_OUTPUT_PATH" "$(dirname $OUTPUT_PATH_GS)/" --gcs-bucket-policy-only
    ...
    ;;
  slurm)
    log_file="$PERF_LOG_DIR/execution_times_growth_stage_inference.csv"
    if [ ! -f "$log_file" ]; then
      header="job_id,job_name,processing_step,num_cores,memory,start_time,end_time,execution_time,output_size"
      echo "$header" > "$log_file"
    fi
